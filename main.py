...
@tasks.loop(time=time(hour=27, minute=28, second=0))  # 10 minutes before daily post
...
@tasks.loop(time=time(hour=15, minute=29, second=0))  # Posts every day at noon UTC
...
@tasks.loop(time=time(hour=15, minute=30, second=0))  # Runs at 23:00 UTC daily
