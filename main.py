...
@tasks.loop(time=time(hour=15, minute=24, second=0))  # 10 minutes before daily post
...
@tasks.loop(time=time(hour=15, minute=25, second=0))  # Posts every day at noon UTC
...
@tasks.loop(time=time(hour=15, minute=26, second=0))  # Runs at 23:00 UTC daily
